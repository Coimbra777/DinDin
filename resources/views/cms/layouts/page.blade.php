  ...
  <div class="wrapper" id="app">
    <header class="main-header">
      ...
      <a href="{{ route('configurations.index') }}" class="logo d-md-flex d-none justify-content-center align-items-center">
        ...
        <span class="logo-lg"><img src="{{ asset('img/logo_cms.svg') }}" alt="{{ config('app.name') }}"></span>
      </a>
      ...
      <nav class="navbar navbar-static-top d-flex d-md-block justify-content-between align-items-center">
        ...
        <!-- <div class="select-cms">
          <span style="color:#fff" class="mr-1">Período:</span>
          <select >
            <option>Selecionar</option>
            <option id="y1">{{ date('Y') - 1 }}</option>
            <option id="y2">{{ date('Y') }}</option>
            <option id="y3">{{ date('Y') + 1 }}</option>
          </select>
          <div> -->
      </nav>
    </header>
