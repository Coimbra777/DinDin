...
- **Projeção (3 meses):** `GET /cms/finance/api/projection` ou `GET /api/projection`. Resposta: `meses[]` (`mes`, `label`, `receitas_previstas`, `despesas_previstas`, `saldo_projetado`) e `meta` (médias base excluindo parcelas, `meses_referencia`, `parcelas_detectadas`). Transações podem ter `installment_number` / `installment_of`; também se infere padrão `3/12` ou “parcela 3 de 12” no título/descrição.
